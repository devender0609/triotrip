...
export default function ResultCard({
  ...
  const dateOut = (outSegs?.[0]?.depart_time || pkg.departDate || "").slice(0, 10);
  const dateRet = (inSegs?.[0]?.depart_time || pkg.returnDate || "").slice(0, 10);
  ...
  const currencyOut: string = pkg.currency || currency;
  ...
  function hotelLinks(h: any, cityFallback: string) {
    ...
    b.searchParams.set("selected_currency", currencyOut);
    ...
    e.searchParams.set("currency", currencyOut);
    ...
    hcx.searchParams.set("currency", currencyOut);
  ...
  function cityHotelFallbacks(city: string, name: string, i: number) {
    ...
    // Prioritize queries that bias to buildings/hotels
  ...
  // ----- compare visuals (single declaration) -----

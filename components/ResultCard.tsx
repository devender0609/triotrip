    ...
    <section className="card" role="region" aria-label={`Option ${index + 1}`}>
      ...
      <footer className="row between">
        <div className="left">
          ...
          {/* Add your Save handler here if wired */}
          <button type="button" className="pill" onClick={() => onSavedChangeGlobal()}>
            ☆ Save
          </button>
        </div>

        {/* Flight & hotel booking links if present on pkg */}
        <div className="right">
          {pkg?.deeplink_flight && (
            <Link href={pkg.deeplink_flight} className="pill" target="_blank" rel="noopener noreferrer">
              Book flight
            </Link>
          )}
          {showHotel && pkg?.deeplink_hotel && (
            <Link href={pkg.deeplink_hotel} className="pill" target="_blank" rel="noopener noreferrer">
              Book hotel
            </Link>
          )}
        </div>
      ...
      <style jsx>{`
        .card {
          background: #fff; border: 1px solid #e5e7eb; border-radius: 14px; padding: 12px; display: grid; gap: 10px;
        }
        ...
        .layover {
          ...
          background: radial-gradient(80% 120% at 0% 50%, #ecfeff, transparent), #fff;
          border: 1px dashed #bae6fd; border-radius: 12px; padding: 10px;
        }
        .dot { width: 10px; height: 10px; border-radius: 50%; background: #0ea5e9; }
        .chip { font-weight: 700; color: #0369a1; }

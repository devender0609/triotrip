...
function CheckoutInner() {
  ...
  useEffect(() => {
    (async () => {
      // ---------- Tiny safety tweak: do nothing if we weren't passed a flightId ----------
      if (!flightId) {
        setOffer(null);
        return;
      }
  ...
  async function placeOrder() {
    try {
      ...
      setErr(null);
      const body = {
        offer_id: flightId, // must be present for Duffel order creation
  ...
  return (
    <main style={{ maxWidth: 880, margin: "20px auto", padding: 16 }}>
      ...
      >
        <h1 style={{ marginTop: 0, marginBottom: 8, fontWeight: 900 }}>
          Checkout
        </h1>
        <p style={{ color: "#475569", marginTop: 0 }}>
          Enter passenger and contact details. We’ll create the order in Duffel
          (sandbox).
        </p>
        ...
        >
          ...
          <div
            style={{ display: "grid", gridTemplateColumns: "120px 1fr 1fr", gap: 8 }}
          >
            <select
              value={title}
        ...
        <button
          onClick={placeOrder}
          disabled={
            loading || !flightId || !email || !first || !last || !dob
          }
          style={{
            height: 44,
            padding: "0 16px",

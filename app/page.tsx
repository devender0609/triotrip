...
export default function Page() {
  ...
  async function runSearch() {
    setLoading(true); setError(null); setHotelWarning(null); setResults(null); setSearchKey(k => k + 1);
    try {
      ...
      setResults(merged);
      setComparedIds([]);
      setHasSearched(true); // ← makes Explore/Savor tabs appear
    } catch (e: any) {
  ...
  const countryName = useMemo(() => extractCountryFromDisplay(destDisplay) || undefined, [destDisplay]);
  ...
  return (
    <div style={{ padding: 12, display: "grid", gap: 14 }}>
      ...
      <form style={s.panel} onSubmit={(e) => { e.preventDefault(); runSearch(); }}>
        ...
        {includeHotel && (
          <div style={{ display: "grid", gap: 12, gridTemplateColumns: "1fr 1fr 1fr" }}>
            <div><label style={s.label}>Hotel check-in</label><input type="date" style={inputStyle} value={hotelCheckIn} onChange={(e) => setHotelCheckIn(e.target.value)} min={departDate || todayLocal} /></div>
            <div><label style={s.label}>Hotel check-out</label><input type="date" style={inputStyle} value={hotelCheckOut} onChange={(e) => setHotelCheckOut(e.target.value)} min={hotelCheckIn ? plusDays(hotelCheckIn, 1) : (departDate ? plusDays(departDate, 1) : plusDays(todayLocal, 1))} /></div>
      ...
      {/* TOOLBAR */}
      <div className="toolbar" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
      ...
      {/* EXPLORE / SAVOR — appear only after Search; each tab toggles to show/hide its panel */}
      {hasSearched && (
        <ExploreSavorTabs
          hasSearched={true}
      ...
      {/* COMPARE — visible section only when user has added >= 2 */}
      {results && comparedIds.length >= 2 && (
        <section
        ...
        >
          ...
          {results
            .filter((r) => comparedIds.includes(r.id))
            .map((pkg, i) => (
              <ResultCard
                key={`cmp-${pkg.id || i}`}
                pkg={pkg}

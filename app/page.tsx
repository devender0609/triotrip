...
/** Local lightweight airport field (avoids strict prop contract mismatches). */
  ...
  async function runSearch() {
    try {
      ...
      const body = {
        ...
        hotelCheckOut: includeHotel ? (hotelCheckOut || (roundTrip ? returnDate : "")) : undefined,
        minHotelStar,
      ...
      if (!resp.ok) throw new Error((j as any)?.error || "Search failed");

      const packages =
        Array.isArray(j)
          ? j
          : Array.isArray((j as any)?.packages)
          ? (j as any).packages

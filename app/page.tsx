...
export default function Page() {
  ...
  // NEW: Explore visibility appears only after a successful flight search
  const [exploreVisible, setExploreVisible] = useState(false);

  // keep children ages array in sync
  useEffect(() => {
  ...
  // turning off hotel clears its dates
  useEffect(() => {
    if (!includeHotel) {
      setHotelCheckIn("");
      setHotelCheckOut("");
  ...
  async function runSearch() {
    ...
    try {
      ...
      // show Explore only after a successful search
  ...
  /* ---------- Derived destination city for Explore content ---------- */
  ...
  function ContentExplore() {
    ...
    return (
      <section className="places-panel" aria-label="Explore destination">
        <div className="subtle-h">🌍 Explore — {destCity}</div>
    ...
    <div className={compareMode ? "compare-mode-on" : undefined} style={{ padding: 12, display: "grid", gap: 14 }}>
      ...
      <section>
        ...
        <p style={{ margin: 0, display: "flex", gap: 10, alignItems: "center", color: "#334155", fontWeight: 700, flexWrap: "wrap", fontSize: 15 }}>
          ...
          <span style={{ opacity: 0.6 }}>•</span><strong>Explore your destination</strong>
      ...
      {/* TOOLBAR (sort chips + fancy Compare + show count) */}
      <div style={s.toolbar}>
        {/* Sort chips */}
      ...
      {/* EXPLORE: appears ONLY after a successful search */}
      {exploreVisible && results && results.length > 0 && <ContentExplore />}

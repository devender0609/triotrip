...
export default function Page() {
  ...
  // pax w/ steppers (readOnly inputs – no spinners)
  ...
  async function runSearch() {
    ...
    try {
      ...
      const payload: SearchPayload = {
        ...
        nights: includeHotel && hotelCheckIn && hotelCheckOut
      ...
      const merged = (Array.isArray(j.results) ? j.results : []).map((res: any) => ({
        ...res,
        ...payload,
      }));
      setResults(merged);
  ...
  const comparedPkgs = useMemo(
    () =>
      results && comparedIds.length
        ? results.filter((r) => comparedIds.includes(r.id || `pkg-${(results as any[]).indexOf(r)}`))
        : [],
    [results, comparedIds]
  );
  ...
  return (
    <div className="main-wrap" style={{ padding: 12, display: "grid", gap: 14 }}>
      {/* tiny global fix for header links */}
      <style jsx global>{`
      ...
      {/* Header: logo left, fancy Saved/Login, currency dropdown */}
      <header
        className="card"
        style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}
      >
        <div className="tt-logo" style={{ fontWeight: 900, fontSize: 20 }}>TrioTrip</div>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <div className="pill pill--ghost" title="Currency">
            <span style={{ fontWeight: 800, marginRight: 6 }}>💱</span>
            <select
              value={currency}
      ...
      <form
        ...
        style={{ maxWidth: 1240, margin: "0 auto", display: "grid", gap: 14 }}
      >
        ...
        {/* Simple filters (refundable/greener removed) */}
        ...
        <div style={{ display: "grid", gap: 12, gridTemplateColumns: "auto 1fr 1fr 1fr" }}>
          ...
          <div>
            ...
            <input type="date" className="in" value={hotelCheckOut} onChange={(e) => setHotelCheckOut(e.target.value)} disabled={!includeHotel}
              min={hotelCheckIn ? plusDays(hotelCheckIn, 1) : (departDate ? plusDays(departDate, 1) : plusDays(todayLocal, 1))} />
          </div>
          <div>
            <label className="lbl">Min stars</label>
      ...
      {tabsVisible && (
        <div className="toolbar">
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            <div className={`toolbar-chip ${sort === "best" ? "toolbar-chip--active" : ""}`} onClick={() => setSort("best")}>✨ Best</div>
            <div className={`toolbar-chip ${sort === "cheapest" ? "toolbar-chip--active" : ""}`} onClick={() => setSort("cheapest")}>💵 Cheapest</div>
            <div className={`toolbar-chip ${sort === "fastest" ? "toolbar-chip--active" : ""}`} onClick={() => setSort("fastest")}>⚡ Fastest</div>
            <div className={`toolbar-chip ${showAll ? "toolbar-chip--active" : ""}`} onClick={() => setShowAll((v) => !v)}>{showAll ? "Top-10" : "Top-3"}</div>
          </div>
        </div>
      )}

      {/* Explore / Savor / Misc — visible on click, row-wise cards */}
      ...
      {/* results (flight + hotel cards keep same width) */}

...
export default function Page() {
  ...
  const [exploreVisible, setExploreVisible] = useState(false); // show only after search
  ...
  async function runSearch() {
    ...
    try {
      ...
      setResults(merged);
      setComparedIds([]);
      setExploreVisible(true); // show Explore/Savor after success
    } catch (e: any) {
            ...
            <thead>
              <tr>
                ...
                {comparedPkgs.map((p: any) => (
                  <th key={p.id} style={cth}>✈️ {p.flight?.carrier_name || "Airline"}</th>
                ))}
              </tr>
            </thead>
            <tbody>
